...
from preprocess import img_2_Torch,resizeImage,cal_ROI
...
if __name__ == '__main__':
    ...
    data_test['ROI'],data_test['X_dst'],data_test['X_org'] = cal_ROI(new_test[0][0][:4],new_test[1][0][:4])
    
    
    train_x = torch.tensor([item.numpy() for item in data_test['X_dst']]).float().cuda().unsqueeze(1)
    train_y = torch.tensor([np.array(item) for item in data_test['ROI']]).long().cuda()
    train_y = train_y.view(len(train_y),6,1,4)
    ...
    for epoch in range(1):
        ...
        for data in train_loader:
            img, label = data
            ROI = label[:,1:].squeeze(0) # ROI区域从第二个向量开始取
            label = label[:,0,:,1] # 所有label，第一个向量， ，第一位
            label = label.view(1,label.size(0)).squeeze(0)
            out= model(img,ROI,3)
            print(out,label)
